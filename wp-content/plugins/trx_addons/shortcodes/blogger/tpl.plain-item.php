...
?><div id="post-<?php the_ID(); ?>"	<?php post_class( 'sc_blogger_item post_format_'.esc_attr($post_format) ); ?>><?php
	...
	<div class="time"><?php
			// Post meta
			echo '<span class="first">'.(strlen(get_the_date('j')) == 1 ? '0' : '').get_the_date('j').'</span>';
			echo '<span class="second">'.get_the_date('F').'</span>';
		?>
	</div>
		<?php
		// Post title
		...
		?><div class="sc_blogger_item_excerpt">
			...
			// More button
			if ( $show_more ) {
				?><div class="sc_blogger_item_button"><a href="<?php echo esc_url($post_link); ?>" class="sc_button"><?php esc_html_e('Read More', 'trx_addons'); ?></a></div><?php
			}
